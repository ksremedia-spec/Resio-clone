...
function ClientDetail({ clientId, onDeleted }: { clientId: string; onDeleted: () => void }) {
  ...
  const { data: roles } = useResource<Array<{ id: string; key: string | null }>>(session.has('members.invite') ? '/v1/roles' : null);
  const [inviteLink, setInviteLink] = useState<string | null>(null);
  const invite = useApiMutation((c: contracts.Contact) => api.mutate<{ acceptUrl?: string }>('POST', '/v1/invitations', { email: c.email, roleId: roles!.find((r) => r.key === 'client')!.id, firstName: c.firstName, lastName: c.lastName, projectIds: (projects?.items ?? []).map((p) => p.id) }), ['/v1/invitations']);
    ...
    <div className="page-inner stack" style={{ gap: 'var(--sp-4)' }}>
      ...
      <div className="card-grid">
        ...
        <Card title="Contacts" actions={canEdit && <Button size="sm" icon="plus" onClick={() => setContactSheet({})}>Add</Button>}>
          {client.contacts?.length ? <div className="list">{client.contacts.map((c) => <div key={c.id} className="list-row" style={{ padding: '8px 0' }}><Avatar name={`${c.firstName} ${c.lastName}`} size="sm" /><span className="grow"><div className="primary">{c.firstName} {c.lastName} {c.isPrimary && <Badge tone="brand">primary</Badge>}</div><div className="secondary">{[c.title, c.email, c.phone].filter(Boolean).join(' · ')}</div></span>{canEdit && <span className="trailing">{c.email && roles && !c.portalUserId && <Button size="sm" variant="quiet" icon="link" aria-label="Invite to client portal" title="Invite to client portal" onClick={() => invite.mutateAsync(c).then((r) => { setInviteLink(r.data.acceptUrl ?? null); toast({ message: `Portal invitation sent to ${c.email}.`, tone: 'success' }); }).catch(errorToast)} />}{c.portalUserId && <Badge tone="success">portal</Badge>}<Button size="sm" variant="quiet" icon="edit" aria-label="Edit contact" onClick={() => setContactSheet({ contact: c })} /><Button size="sm" variant="quiet" icon="trash" aria-label="Remove contact" onClick={() => removeContact.mutate(c.id)} /></span>}</div>)}</div> : <p className="muted">No contacts yet.</p>}
        </Card>
      ...
      {inviteLink && <Sheet open onClose={() => setInviteLink(null)} title="Portal invitation" footer={<Button variant="primary" onClick={() => setInviteLink(null)}>Done</Button>}><p>The homeowner can open this link to set a password and sign in to their portal. It was also emailed to them.</p><Input readOnly value={inviteLink} onFocus={(e) => e.currentTarget.select()} data-testid="portal-invite-link" /></Sheet>}
