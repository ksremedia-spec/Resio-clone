...
function ChangeOrderDetail({ project, coId, onClose }: { project: contracts.ProjectDetail; coId: string; onClose: () => void }) {
  ...
  const decidable = ['sent', 'viewed', 'draft', 'pending_internal'].includes(co.status);
  return <Sheet open onClose={onClose} title={`Change order #${co.number}`} size="lg" footer={<>
    ...
    {canWrite && decidable && <Button onClick={() => setDeciding('declined')}>Decline</Button>}
    {canWrite && decidable && <Button variant="primary" icon="check" onClick={() => setDeciding('approved')}>Record approval</Button>}
  </>}>
    ...
    <div className="table-wrap"><table className="table">
      <thead><tr><th>Line</th><th className="num">Qty</th><th className="num">Unit cost</th><th className="num">Cost</th><th className="num">Sell</th></tr></thead>
      <tbody>{co.lines.map((l) => <tr key={l.id}><td className="name"><div className="primary">{l.name}</div><div className="secondary">{[l.budgetLineName ? `Budget: ${l.budgetLineName}` : 'New budget line on approval', l.taxable ? 'taxable' : ''].filter(Boolean).join(' · ')}</div></td><td className="num">{formatQuantity(l.quantityThousandths)} {l.unit}</td><td className="num"><Money cents={sumCost(l.unitCostCents)} /></td><td className="num"><Money cents={l.costCents} /></td><td className="num"><Money cents={l.sellCents} /></td></tr>)}</tbody>
    </table></div>
    <div className="totals mt-4">
      <div className="row-between"><span className="muted">Cost</span><Money cents={co.costCents} /></div>
      <div className="row-between"><span className="muted">Markup</span><Money cents={co.markupCents} /></div>
    ...
    {deciding && <DecisionSheet coId={coId} projectId={project.id} decision={deciding} onClose={() => setDeciding(null)} onSaved={() => void refetch()} />}
...
function DecisionSheet({ coId, projectId, decision, onClose, onSaved }: { coId: string; projectId: string; decision: 'approved' | 'declined'; onClose: () => void; onSaved: () => void }) {
  const toast = useToast();
  const errorToast = useErrorToast();
  const [form, setForm] = useState({ decidedByName: '', note: '' });
  const save = useApiMutation(() => api.mutate('POST', `/v1/change-orders/${coId}/decide`, { decision, decidedByName: form.decidedByName.trim(), note: form.note || undefined }), [...inv(projectId), `/v1/change-orders/${coId}`]);
  return <Sheet open onClose={onClose} title={decision === 'approved' ? 'Record approval' : 'Record decline'} footer={<><Button onClick={onClose}>Cancel</Button><Button variant={decision === 'approved' ? 'primary' : 'danger'} loading={save.isPending} onClick={async () => { if (!form.decidedByName.trim()) { toast({ message: 'Enter who made the decision.', tone: 'error' }); return; } try { await save.mutateAsync(undefined); toast({ message: decision === 'approved' ? 'Approved. The contract and budget are updated.' : 'Declined.', tone: 'success' }); onClose(); onSaved(); } catch (e) { errorToast(e); } }}>{decision === 'approved' ? 'Approve' : 'Decline'}</Button></>}>
    <div className="form-grid">
      <Field label="Decided by" className="full" hint="The client contact who signed or replied."><Input value={form.decidedByName} onChange={(e) => setForm({ ...form, decidedByName: e.target.value })} autoFocus placeholder="Jane Smith" /></Field>
      <Field label="Note" className="full"><Textarea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="Signed copy in the Contracts folder" /></Field>
    </div>
    <p className="subtle"><Icon name="info" size={14} /> The decision is written to the permanent approval record with your name and time.</p>
  </Sheet>;
}
