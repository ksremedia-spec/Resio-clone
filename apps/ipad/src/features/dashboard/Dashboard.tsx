...
const jumpTo = (id: string) => { const el = document.getElementById(id); if (el) { el.scrollIntoView({ behavior: 'smooth', block: 'start' }); el.classList.add('flash'); setTimeout(() => el.classList.remove('flash'), 1200); } };
  ...
  return (
    <div className="stack" style={{ gap: 'var(--sp-4)' }}>
      <div className="stat-row">
        <Stat label="Active projects" value={data.activeProjects.count} hint="See all projects" onClick={() => navigate('/projects')} />
        {data.overdueTasks && <Stat label="Overdue tasks" value={data.overdueTasks.count} tone={data.overdueTasks.count ? 'danger' : 'ok'} hint="Open the task manager" onClick={() => navigate('/tasks?scope=all')} />}
        {data.upcomingDeadlines && <Stat label="Due this week" value={data.upcomingDeadlines.count} hint="Open the task manager" onClick={() => navigate('/tasks?scope=all')} />}
        {data.pendingApprovals && <Stat label="Pending approvals" value={data.pendingApprovals.count} tone={data.pendingApprovals.count ? 'warn' : undefined} hint="Jump to the list" onClick={() => jumpTo('dash-approvals')} />}
        {data.unpaidInvoices && <Stat label="Unpaid invoices" value={money(data.unpaidInvoices.totalCents)} tone={data.unpaidInvoices.count ? 'warn' : undefined} hint="Open invoices" onClick={() => navigate('/invoices?status=open')} />}
        {data.scheduleConflicts && <Stat label="Schedule conflicts" value={data.scheduleConflicts.count} tone={data.scheduleConflicts.count ? 'danger' : 'ok'} hint={data.scheduleConflicts.count ? 'Jump to the list' : 'Open the schedule'} onClick={() => (data.scheduleConflicts!.count ? jumpTo('dash-conflicts') : navigate('/schedule'))} />}
      </div>
      ...
      <div className="card-grid">
        ...
        {data.pendingApprovals && <Card title="Pending approvals" id="dash-approvals">
        ...
        {data.scheduleConflicts && data.scheduleConflicts.count > 0 && <Card title="Schedule conflicts" id="dash-conflicts">
