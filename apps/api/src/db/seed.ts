...
export async function seed(opts: { log?: (m: string) => void } = {}) {
  ...
  const { services } = app;
  const db = (services as any).auth['deps'].db as import('./client.js').Db;
  try {
    const [existing] = await db.select({ id: users.id }).from(users).where(eq(users.email, DEMO_OWNER.email)).limit(1);
    if (existing) { log('demo data already present; skipping'); return; }
