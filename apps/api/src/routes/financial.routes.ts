...
export async function financialRoutes(app: AppInstance, s: Services) {
  ...
  app.get('/selections/template', { schema: { tags: por, response: { 200: contracts.selectionTemplateSchema } } }, async (req) => s.selections.template(req.requireCtx()));
  app.post('/projects/:id/selections/apply-template', { schema: { tags: por, params: pid, body: contracts.applySelectionTemplateBody.nullish(), response: { 200: contracts.applySelectionTemplateResponse } } }, async (req) => s.selections.applyTemplate(req.requireCtx(), req.params.id, req.body ?? { release: false }));
  app.get('/projects/:id/selections/sheet', { schema: { tags: por, params: pid, response: { 200: contracts.selectionSheetSchema } } }, async (req) => s.selections.sheet(req.requireCtx(), req.params.id));
  app.post('/projects/:id/selections/publish', { schema: { tags: por, params: pid, body: contracts.publishSelectionSheetBody.nullish(), response: { 201: contracts.documentSchema } } }, async (req, reply) => reply.status(201).send(await s.selections.publish(req.requireCtx(), req.params.id, req.body ?? { vendorVisible: true })));
  app.post('/projects/:id/selections/sign', { schema: { tags: por, params: pid, body: contracts.signSelectionSheetBody, response: { 200: contracts.selectionSheetSchema } } }, async (req) => s.selections.sign(req.requireCtx(), req.params.id, req.body));
