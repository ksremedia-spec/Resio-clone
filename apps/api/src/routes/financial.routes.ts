...
export async function financialRoutes(app: AppInstance, s: Services) {
  ...
  app.post('/invoices/:id/payment-intent', { schema: { tags: inv, params: pid, body: z.object({ method: z.enum(['card', 'ach']).default('card') }).nullish(), response: { 200: z.object({ id: z.string(), clientSecret: z.string().optional(), checkoutUrl: z.string().optional(), status: z.string() }) } } }, async (req) => s.invoices.createPaymentIntent(req.requireCtx(), req.params.id, req.body?.method ?? 'card'));
  void ok;
}
